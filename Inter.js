 ...
 const someRef={
   ...
   let father=parent.children[ind];
       if(containChild(parent.children[ind])){
        if(father.children.length!=value.children.length){
...
function InterpretAttrs(el,attrs, attrManager){
    
Object.entries(attrs).reduce(($,attr)=>{
     const[attrName, attrValue]=attr;
         if(attrValue!=undefined && attrValue!=null && !attrName.startsWith("on")){
...
function findDynamicAttrs(rootElem, attrManager, dynamic){
...
for(let child of children){
    ...
    if(child.attributes.length==1){
        //when dynamic is true.
         const theAttrValue=child.attributes[0].value;
       ...
       if(!dynamic){
        ...
        if(theAttrValue==key){
            const copy=Object.assign({},attrManager[key])
            child.removeAttribute("attributesmanager");
             InterpretAttrs(child,copy,key);
...
function toATTR(obj){
  ...
  if(!isPlainObject(data)){
   ...
   findDynamicAttrs(root(),data, plainEl);
  }      
    }
}

/**
 * Let's introduce  a major concept in Inter v1.1.0
 *  that will give the opportunity to work with complex
 * template easier and smother.
 *
 */
  ...
  * elements:[{
  * tag:"div", children:[{
  * tag:"h2", text:`${com.length} Comentary`
  * }]
  * }]
  * })
  * 
  */
  ...
  Array.from(elements).forEach(el=>{
      ...
      const htmlElement=CreatEL(tag);
    Object.entries(attrs).reduce((theEl,attr)=>{
       const[attrName,attrValue]=attr;
       if(isCallable(attrValue)){
           setAttr(theEl,attrName, attrValue());
       }else{
           setAttr(theEl, attrName, attrValue);
       }
    },htmlElement)
    Object.entries(events).reduce((theEl,_event)=>{
   const[attrName,attrValue]=_event;
   theEl[attrName]=attrValue;
    },htmlElement);
    [...children].forEach(child=>{
        const{
            tag,
        ...
        theEl[attrName]=attrValue;
         ...
         if(lowerCase.call(childElement.nodeName)!=null && lowerCase.call(childElement.nodeName)!=undefined){
            htmlElement.appendChild(childElement);
         }
    })
if(lowerCase.call(htmlElement.nodeName)!=null && lowerCase.call(htmlElement.nodeName)!=undefined){
    root.appendChild(htmlElement);
    Object.values(mounted).forEach(fn=>{
     if(isCallable(fn)){
         fn.call(IN);
     }
    })
