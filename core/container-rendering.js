  ...
  return {
    render(pathToContainer, info) {
      ...
      getTheContainer(
        ...
        root,
        info,
        isCallable(mounted) ? mounted : null
      );
    },
  };
}
...
function getTheContainer(pathToContainer, rootEl, info, mountedEvent) {
  ...
  request.onerror = () =>
    err(`Inter was unable to load the container at "${pathToContainer}"`);

  request.send(null);
}

function parseStringToLang(
  __string,
  __rootEl,
  __pathToContainer,
  __info,
  ...
  const __styleRule = /<style>(:?[\s\S]+)<\/style>/gi,
    __scriptRule = /<script>(:?[\s\S]+)<\/script>/gi,
    __template = /<container>(:?[\s\S]+)<\/container>/gi;
  ...
  __string.replace(
    __styleRule,
    (__match) =>
      (__css = __match.replace("<style>", "").replace("</style>", ""))
  );
  __string.replace(
    __scriptRule,
    (__match) =>
      (__js = __match.replace("<script>", "").replace("</script>", ""))
  );
  __string.replace(
    __template,
    (__match) =>
      (__html = __match.replace("<container>", "").replace("</container>", ""))
  );
