...
export function staticList(options) {
  ...
  const root = getId(IN);
  const listParser = {
    foundForEach: new Set(),
    get [Symbol.toStringTag]() {
      return "listParser";
    },
    findForEach(node /*HTML Element*/) {
  ...
  function list(iterator /*plain object*/) {
    ...
    if (_forEach.trim().length == 0) {
      ParserWarning(`
            ...
            `);
    } else if (!hasOwnProperty(data, _forEach)) {
      ParserWarning(`
      ...
      for (const item of iterationData) {
        ...
        if (!isObj(__refs)) {
          syErr(`
                ...
                The "refs" method must return a plain Javascript object, and not "${valueType(
                  __refs
                )}".
        ...
        const producedHTML = domParser.parseFromString(
          parsedTemplate,
          "text/html"
        ).body.children[0];
