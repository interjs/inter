...
import {
  type conditionalAttributeCounterInterface,
  nodeTypes,
  type parserOptionsInterface,
...
export function getChildNodes(root: Element): customizedElementInterface[] {
  ...
  root.childNodes.forEach((node) => {
    ...
    if (isAnElementNode || (isATextNode && hasText)) {
      nodes.push(node);
    }
  });
...
export function hasMoreThanOneConditionalAttribute(
  elementNode: Element
): boolean {
